...
dTree.prototype.node = function(node, nodeId) {
	var showSub = '';
	if (this.config.submenu) showSub = ' onmouseover="unhideTreeMenu(\'op'+(parseInt(node.id)+1)+'\');"  onmouseout="hideDiv(\'op'+(parseInt(node.id)+1)+'\');"';
	var str = '<div class="dTreeNode"'+showSub+'>' + this.indent(node, nodeId);
	...
	if (node.url) {
		...
		if (this.config.useStatusText) str += ' onmouseover="alert(\''+ this.obj + nodeId +'\');window.status=\'' + node.name + '\';return true;" onmouseout="window.status=\'\';return true;" ';
	...
	{
		str += this.nodeMenu(node.id,node.pid);
	}
	str += '</div>';
	if (node._hc) {
...
dTree.prototype.indent = function(node, nodeId) {
	...
	if (this.root.id != node.pid) {
		...
		if (node._hc) {
			...
			str += '" alt="" /></a>';
			//str += '<img src="' +  ((this.config.useLines) ?  this.icon.hline : this.icon.empty) + '" alt="" />';
		} else {
...
dTree.prototype.deleteHighlightCookie = function(){
	var now = new Date();
	var yesterday = new Date(now.getTime() - 1000 * 60 * 60 * 24);
	this.setCookie('cs'+this.obj, 'cookieValue', yesterday);
}
...
dTree.prototype.nodeMenu = function(nodeId,parentId){
	// node menu for operations
	nodeId = nodeId + 1;
	parentId = parentId + 1;
	var str = '';
